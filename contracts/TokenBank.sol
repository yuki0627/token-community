...
contract TokenBank {
    ...
    /// @dev NFTメンバーのみ
    modifier onlyMember() {
        require(imemberNFT.balanceOf(msg.sender) > 0, "not NFT member");
        _;
    }
    ...
    function transfer(address to, uint256 amount) public onlyMember {
        if(owner == msg.sender) {
            require(_balances[owner] - _bankTotalDeposit >= amount, "The amounts grater than the total supply cannot be transferred");
        }
    ...
    function withdraw(uint256 amount) public onlyMember notOwner {
        ...
        require(toTokenBankBalance >= amount, "The amount grater than your tokenBank balance!");
        _transfer(from, to, amount);
        _tokenBankBalances[to] -= amount;
        _bankTotalDeposit -= amount;
        emit TokenWithdraw(from, amount);
    }
}